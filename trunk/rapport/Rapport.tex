...
Notre deuxième interface est directement dans le terminal et se présente sous forme d'un questionnaire à choix multiples. On peut ainsi réupérer des informations ou modifier les tables. Le schéma suivant montre le menu principal :\\
...
Les premiers onglets permettent de visualiser les différentes tables. Ceux-ci n'ont pas été mis dans la section ``chercher des information précise'' car on a souvent besoin de les consulter afin de vérifier par exemple l'effet d'une requête sur une table.\\
...
Lorsque c'est necessaire, l'utilisateur peut choisir l'entité concernée par sa requête(comme une date ou un club).\\

En outre, la plupart du temps une option est proposée à l'utilisateur afin de revenir en arrière dans ses choix si jamais il s'est trompé.\\

Au final cette interface offre toutes les fonctionnalités de l'interface graphique. \\
